...
def createId(user,ts=0):
    ...
    else: return user *100000000000 + ts
...
def splitter(conf):
    ...
    key = mybucket.get_key(path.join(conf['split']['split'], conf['split']['name']+'_$folder$'))
    ...
    if mode == 'ts-1' or mode == 'ts-multi':
        ...
        splitTestTrain.filter(lambda x: long(x[0]) <= TS).map(lambda x: json.dumps(x[1])).saveAsTextFile(path+"train/batchTraining/")
