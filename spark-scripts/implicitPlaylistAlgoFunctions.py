...
def getValidSessionIds(train,sessionJaccardShrinkage,clusterSimilarityThreshold):
    ...
    validSessionIds = gusPerUserAggregated.map(lambda x: filterSessionsByJ(computeIntraSessionJaccardOriginalSessionId(x[1], sessionJaccardShrinkage), clusterSimilarityThreshold)).collect()
...
def getImplicitPlaylists(train,validSessions):
    ...
    return events_session_filtered.map(lambda line: extractTracksListFromEvent(line)).zipWithIndex().flatMap(lambda x: unrollPlaylists(x))
